...
module.exports = (options, app) => {
	return async function saveAndReadCacheMiddleware(ctx, next) {
		...
		const prekey = app.config.customCache.prekey || 'xx';
		...
		if (type == 'redis') { // 读取外部redis存储缓存
			const urlResult = await app.redis.get(url); // 字符串
			if (urlResult) {
				const readObj1 = JSON.parse(urlResult);
			...
			if (app.cache[url]) {
				const readObj = JSON.parse(app.cache[url]);
		...
		// 存储相应缓存
		options = options || {};
		const outTime = options.outTime || app.config.customCache.outTime || 600;
