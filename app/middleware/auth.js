...
module.exports = (options, app) => {
	return async function authMiddleware(ctx, next) {
		...
		const projectRunName = !!ctx.app.config.projectRunName ? '/' + ctx.app.config.projectRunName : '';
		...
		if (ctx.session.userinfo) {
			...
			const checkUrls = options.noneedLoginUrls || [];
			const isnoneed = checkUrls.some(item => {
				if ((projectRunName + item) == pathname || (projectRunName + item + '/') == pathname) {
					return true;
				} else {
					return false;
				}
